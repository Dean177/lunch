...
describe('createLocalStorageMergeReducer', () => {
  const SomeActionType: string = 'SomeAction';
  const stateKey = 'stateKey';
  interface TestState {
    aKey: string;
    bKey: string;
  }
  const initialState: TestState = { aKey: 'initialA', bKey: 'initialB' };
  const reducerObject = {
    [SomeActionType](state: TestState, action: Action) {
  ...
  it('Will store its state in localStorage using the provided state key after each action', () => {
    ...
    chai.expect(localStorage.getItem(stateKey)).to.equal(JSON.stringify(initialState));
  });
  ...
  it('Will attempt to get its initial state from localStorage, falling back to the provided initial state', () => {
    ...
    const savedState = reducer(undefined, { type: '', payload: {}, meta: {} });

    chai.expect(savedState).to.deep.equal(localStorageState);
  });

  it('Will merge state returned by the reducerObject into its current state to produce a new state object', () => {
    const action = { type: SomeActionType, payload: { key: 'bKey', value: 'newBValue'}, meta: {} };
    const reducer = createReducer(mockLocalStorage());
    const nextState = reducer(initialState, action);
    chai.expect(nextState).to.deep.equal({ aKey: initialState.aKey, bKey: action.payload.value });
  });
  ...
  it(`Will fall back to using initial state where the retrieved localStorage doesn't match the shape of the initial state`, () => {
    ...
    chai.expect(savedState).to.deep.equal(initialState);
  });
});
