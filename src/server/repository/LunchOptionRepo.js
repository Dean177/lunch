...
const db = require('./db');
const logger = require('../logger')('LunchOptionRepo');

export const isExistingOption = (optionName) => db('lunch_options')
    .where('name', 'ilike', optionName)
    .select()
    .limit(1)
    .then((existingOptions) => (existingOptions.length === 1 ? existingOptions[0] : false));

export const updateLastChosen = (lunchOptionId) => db('lunch_options')
    .where({ id: lunchOptionId })
    .update({ lastChosen: new Date().getTime() })
    .then(() => lunchOptionId);

export const add = (name) => isExistingOption(name).then((existingOption) => {
  if (existingOption) {
